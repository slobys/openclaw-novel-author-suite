---
name: novel-author
description: 使用 Novel Engine 0.6.0 Balanced-Fast，以逐章隔离 Writer、并行低 Token 独立审稿、可取消状态机、范围完整性检查和可恢复单工具收尾创作、续写、审计及版本化修订中文长篇小说。
---
...
普通非检查点章节由 Finalize 执行 `scope=chapter` 的本章及本章派生绑定检查，不再额外扫描全书；第 1 章、每第 5 章、卷边界、关键章、异常恢复和修订后执行 `scope=project` 完整检查。`repair=true` 只修复可确定的 Meta/State，不伪造语义事实。
...
每个用户生产回合默认只完成一章，不自动开始下一章。主会话默认只报告章节号、标题、汉字数、Audit、Quality、Commit、Closure、Integrity scope 和下一章号；用户要求全文时再读取或导出，禁止在进度消息中复述正文、完整审稿 JSON 或完整工具 Payload。
