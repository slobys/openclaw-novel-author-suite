...
grep -q 'V5.4.2' "${OPENCLAW_STATE_DIR}/workspace-novel-author/AGENTS.md"
...
grep -q 'plugins install git:github.com/slobys/openclaw-novel-author-suite@v0.4.9 --force --accept-capabilities' "${FAKE_OPENCLAW_LOG}"
...
grep -q 'config set agents.entries.novel-author.tools.alsoAllow .*group:fs.*group:runtime.*novel-engine.*sessions_spawn.*--strict-json' "${FAKE_OPENCLAW_LOG}"
...
grep -q '^plugins install git:github.com/slobys/openclaw-novel-author-suite@v0.4.9 --force$' "${FAKE_OPENCLAW_LOG}"
...
grep -q 'config set agents.entries.novel-author.tools.allow .*group:fs.*group:runtime.*novel-engine.*sessions_spawn.*--strict-json' "${FAKE_OPENCLAW_LOG}"
