...
assert asset['schema_version']=='3.3.0'
...
assert anchor['schema_version']=='3.3.0'
assert 'Anchor-A' in anchor['anchors']
assert 'GREEN' in anchor['coverage_logic']
pipeline=json.loads((root/'assets'/'pipeline-output-spec.template.json').read_text(encoding='utf-8'))
assert pipeline['schema_version']=='3.3.0'
assert set(pipeline['passes'])=={'BASE_ASSET','SHOT_ASSET_GAP'}
