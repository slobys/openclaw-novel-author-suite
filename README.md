...
面向 OpenClaw 的长篇小说全流程套件：`Novel Engine 0.6.0` 持久化插件 + `Novel Author V6.1 Balanced-Fast` Agent Workspace。0.6.0 保留原有门禁、数据格式与可恢复收尾，同时减少重复资料读取、串行等待和每章全书扫描。
...
```bash
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/installer-v1.4.6/setup.sh | bash
```
...
```bash
# 只安装小说创作版
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/installer-v1.4.6/setup.sh | bash -s -- 1

# 只安装小说转 AI 漫剧版
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/installer-v1.4.6/setup.sh | bash -s -- 2

# 无交互确认：安全卸载小说创作版
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/installer-v1.4.6/setup.sh | OPENCLAW_SUITE_CONFIRM_UNINSTALL=1 bash -s -- 3

# 无交互确认：安全卸载小说转 AI 漫剧版
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/installer-v1.4.6/setup.sh | OPENCLAW_SUITE_CONFIRM_UNINSTALL=1 bash -s -- 4
```
...
| 流程节点 | 完整含义 |
| --- | --- |
| 1 精简准备 | 读取服务端 `nextChapter`，建立一次两分钟有效的章节资料快照，再从同一快照生成三个角色包；普通章不重复读取全套资料 |
...
| 5–6 独立审稿 | Continuity Auditor 与 Reader Editor 使用两个真实且不同的精简子会话；最终正文 Hash 固定后并行执行，普通章默认低思考强度 |
...
| 11 完整性 | 普通章只验当前章及其派生记录；第1章、每5章检查点、严格模式或故障诊断再扫全项目；只有通过才把 `nextChapter` 交给下一章 |
...
OpenClaw 的叶子子会话默认收窄会话和消息工具，这是正常安全边界。V6 不要求 Writer/Reviewer 自己访问文件、执行命令、调用 `novel_*` 或创建子会话：它们只返回一个严格 JSON，主会话用真实 completion、真实 session ID 和正文内容生成文件与 Hash。安装器会同时写入 `group:fs`、`group:runtime` 以及 `read/write/edit/apply_patch/exec/process` 具体 ID，确保 Codex Harness 能把本地工具真实投射给父会话。

### V6.1 Balanced-Fast 与单工具收尾

普通章节默认使用以下节流规则：

| 项目 | Balanced-Fast 行为 |
| --- | --- |
| 资料读取 | 同一章节阶段只建一份快照，Writer 与两个 Reviewer 复用 |
| Writer 资料包 | 最多 16,000 字符 |
| Continuity 资料包 | 最多 8,000 字符 |
| Reader 资料包 | 最多 6,000 字符 |
| 审稿调度 | 两个 Reviewer 在最终正文 Hash 固定后并行 |
| 通过项 | 仅写 `"pass"`，证据和修订建议只放 `issues` |
| Integrity | 普通章查当前章；第1章及每5章查全项目 |

`strict` 模式仍可用于发布前、重大修订或故障排查。Balanced-Fast 只减少重复工作，不降低必需检查数量。

`novel_finalize_chapter` 把下列服务端操作收拢为一次调用：

```text
Audit -> Quality -> Commit -> 台账/状态/记忆 -> Closure -> Integrity
```

它不会减少检查项目，也不会信任模型口头声称。所有数据仍由原有 Engine 方法重新检查。中断后使用同一 `requestId` 再次调用时，工具先对账 Commit 和正文 Hash；已经提交的正文不会重写，后续幂等台账和 Closure 可以继续完成。

这里的保证是“可恢复且幂等”，不是虚构的跨文件数据库原子事务。
...
```bash
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/v0.6.0/install.sh | bash
```

安装器会：

1. 从固定标签 `v0.6.0` 安装并启用 `novel-engine`，OpenClaw 2026.8.1+ 会显式完成插件能力授权；
2. 把公共 Agent 模板部署到 `~/.openclaw/workspace-novel-author`；
3. 已存在的同名 Workspace 文件先备份，不覆盖 `memory/`、`exports/`、`.novel-runtime/` 或小说数据；
4. 创建或更新 `novel-author` Agent，并保留现有工具白名单后补齐策略组及 Codex 所需的具体文件/命令工具 ID；
...
```text
继续创作项目 <projectId> 的下一章。以服务端 nextChapter 为准，确认上一章 Closure 和 Integrity 后按 Balanced-Fast 流程执行。
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/slobys/openclaw-novel-author-suite/v0.6.0/uninstall.sh | bash
```
...
```bash
openclaw plugins install git:github.com/slobys/openclaw-novel-author-suite@v0.6.0 --force --accept-capabilities
...
- [0.6.0 升级说明](UPGRADE-0.6.0.md)
- [0.5.0 升级说明](UPGRADE-0.5.0.md)
- [0.4.10 兼容基线说明](UPGRADE-0.4.10.md)
- [安全策略](SECURITY.md)
